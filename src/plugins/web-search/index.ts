...
import { WEB_SEARCH_INPUT_SCHEMA, extractSearchQueryInput, summarizeToolInput, type WebSearchToolInput } from './adapter.js';
import { buildSearchPlan } from './planner.js';
import { runSearch } from './orchestrator.js';
import type { SearchProviderId, SearchResult, SearchRuntimeConfig } from './types.js';
...
const DEFAULT_PROVIDERS: SearchProviderId[] = ['bing-web', 'duckduckgo-lite', 'wikipedia', 'moegirl'];
...
export const Config: Schema<Config> = Schema.object({
  ...
  acgnExtensionEnabled: Schema.boolean().default(true).description('是否启用 ACGN 扩展 provider。'),
});
...
class WebSearchTool extends StructuredTool<typeof WEB_SEARCH_INPUT_SCHEMA, WebSearchToolInput, WebSearchToolInput, string> {
  name = 'web_search';
  description =
    'Use this whenever you need current facts, unfamiliar entities, or verification. Search first instead of guessing, then return a JSON array with title, url, description, and optional content.';
