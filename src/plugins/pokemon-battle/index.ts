...
import { resolvePokemonCommandRoute } from './route.js';

export const name = 'pokemon-battle-bridge';
export const inject = { required: ['canvas'], optional: ['featurePolicy'] } as const;
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (featurePolicy && !(await featurePolicy.resolveFeatureEnabled(session as Session, 'POKEMON_BATTLE_ENABLED'))) {
      return next();
    }
