...
import { formatNaturalRunAtText } from './scheduler.js';
...
export const DEFAULT_DELIVERY_SYSTEM_PROMPT =
  ...
  '7) 普通提醒/确认/闲聊/安慰/解释优先写成一条连续消息；只有列表、代码、命令、配置、日志或分步骤结果才允许保留必要换行；' +
  '8) 即使需要换行，也只输出纯文本正文，不要自造标签、控制符或额外协议；' +
  '9) 不要把同一句话重复改写成两种发送形式，也不要输出“下面分点说”“我来发语音”之类的自我说明；' +
