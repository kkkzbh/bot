...
import { buildMemoryExtractionPrompt, parseMemoryExtractionJson, type MemoryConversationTurn } from './schemas.js';
...
async function requestJsonMode(profile: MemoryProviderProfile, turns: MemoryConversationTurn[]): Promise<string> {
    ...
    const response = await fetch(`${profile.baseUrl.replace(/\/$/, '')}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: buildMemoryExtractionPrompt(turns, 'json_mode_with_repair') },
        ],
      }),
...
export async function requestJsonModeMemoryWithRepair(
  ...
  const rawText = await requestJsonMode(profile, turns);
  try {
    ...
    const repairText = await requestChatMemoryPlainText(
      ...
      [
        ...turns,
        {
          ...
          role: 'human',
          text: [
            '上一次输出不是可解析的 memory_extraction_v3 JSON。',
